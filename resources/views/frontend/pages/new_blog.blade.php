...
@section('content')
    ...
    <div data-aos="fade-down" class="max-w-[2500px] w-full bg-slate-300 grow text-white">
        ...
        <div data-aos="fade-down" class="w-full font-sec text-xl text-center">Daily Updates & News</div>
        <div data-aos="fade-down" class="sm:w-3/7 flex text-4xl sm:text-6xl font-pri font-black justify-center text-center font-pri">Our Blog </div>
            <div data-aos="fade-down" class="w-full sm:px-20 flex flex-wrap">
                @foreach($posts as $post)
                <div data-aos="fade-down" class="sm:w-1/3 p-5 flex flex-col group">
                    <div data-aos="fade-down" class="relative" style="height: 603px;">
                        <img src="{{ asset('uploads/post/'.$post->image) }}" class="w-full h-full object-cover" alt="{{ $post->title }}">
                        <div data-aos="fade-down" class="absolute top-0 text-white group-hover:bg-[#FF9933] bg-black duration-300 py-3 px-7 font-bold">
                            <div>{{ $post->category->name ?? 'Travel' }}</div>
                        </div>
                    </div>
                    <div data-aos="fade-down" class="py-3 pt-5 sm:pt-10 uppercase">{{ str_replace('_', ' ', $post->slug) }}</div>
                    <div data-aos="fade-down" class="text-xl font-bold border-b py-1 sm:w-4/5">{{ $post->title }}</div>
                    <div data-aos="fade-down" class="w-4/5 py-5">{{ Str::limit($post->short_content, 150) }}</div>
                    <div data-aos="fade-down" class="w-full sm:justify-start justify-end flex mt-auto">
            ...
            <div data-aos="fade-down" class="w-full flex justify-center mt-10">
